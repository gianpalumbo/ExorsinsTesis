...
public class EliteEnemy : MonoBehaviour
{
    ...
    [SerializeField] GameObject slamGround;
    ...
    bool hasBeenInstantiated = false; //Para saber si ya se instancio el enemigo, para no volver a instanciarlo
    ...
    private State<EliteEnemyInputs> idle, tankFollow, slowAttack, rest, frenzyScream, frenzyFollow, frenzyAttack, frenzyHeal, hitStun, die;
    ...
    private void Awake()
    {
        ...
        idle = new State<EliteEnemyInputs>("IDLE");
        tankFollow = new State<EliteEnemyInputs>("TANKFOLLOW");
        slowAttack = new State<EliteEnemyInputs>("SLOWATTACK");
        rest = new State<EliteEnemyInputs>("REST");
        frenzyScream = new State<EliteEnemyInputs>("FRENZY");
        frenzyFollow = new State<EliteEnemyInputs>("FRENZYFOLLOW");
        frenzyAttack = new State<EliteEnemyInputs>("FRENZYATTACK");
        frenzyHeal = new State<EliteEnemyInputs>("FRENZYHEAL");
        hitStun = new State<EliteEnemyInputs>("HITSTUN");
        die = new State<EliteEnemyInputs>("DIE");
        #endregion
        ...
        StateConfigurer.Create(frenzyScream) //ESTE VA A SER COMO UN IDLE, QUE CADA 3 ATAQUES GRITE Y VUELVA A ATACARTE PARA GENERAR UNA PEQUE�A VENTANA PARA ATACAR
        ...
        tankFollow.OnUpdate += () =>
        {
            ...
            if (GetDistanceToParameterFrom(transform, _player.transform) > _followRadiusSlow) SendInputToFSM(EliteEnemyInputs.REST);
        };
        ...
        #region REST
        rest.OnEnter += x => { _anim.SetBool("isWalking", false); };
        rest.OnUpdate += () =>
        {
            if ((_enemyLife.Life / _enemyLife.MaxLife) < .5f) SendInputToFSM(EliteEnemyInputs.FRENZY); //SI TENGO MENOS DEL 50% ENTRO EN MODO FRENZY
            if (GenericCounter(timeToRest) && GetDistanceToParameterFrom(transform, _player.transform) < _followRadiusSlow) //con el genericCounter usar resetCounter
            {
                SendInputToFSM(EliteEnemyInputs.TANKFOLLOW);
            }
        };
        ...
        {
            if (_enemyLife.Life <= 0)
            {
                SendInputToFSM(EliteEnemyInputs.DIE);
                return;
            }
            animationFinished = false;
    ...
    void HitboxOn() {
        SoundManager.Instance.PlayOneShotFromIndex(10);
        slamGround.SetActive(true);
        slowAttackPrefab.SetActive(true); 
    }
    void HitboxOff() {
        slowAttackPrefab.SetActive(false); 
    }
    void SlamGroundOff() => slamGround.SetActive(false);
